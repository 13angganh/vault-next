    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', width: '100%' }}>
        ...
        {stepLabels.map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < stepLabels.length - 1 ? 1 : undefined }}>
            ...
            {i < stepLabels.length - 1 && (
              <div style={{ flex: 1, height: 1.5, background: i < stepIdx ? 'var(--gold)' : 'var(--border)', transition: 'background var(--transition-fast)' }} />
            )}
          </div>
        ))}
      </div>
      ...
      {step === 'password' && (
        <div style={{ animation: 'fadeUp 0.3s ease both' }}>
          <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>Buat Master Password</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div style={{ position: 'relative' }}>
              <input type={showPw ? 'text' : 'password'} value={pw}
                ...
                placeholder="Master password…" autoFocus style={inputStyle(!!error && pw.length > 0)} />
              ...
              <div style={{ animation: 'fadeIn 0.2s ease' }}>
                <div style={{ display: 'flex', gap: 3, marginBottom: 4 }}>
                  {Array.from({ length: 7 }, (_, i) => (
                    <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < pwStrength.level ? pwStrength.color : 'var(--border)', transition: 'background 0.2s ease' }} />
                  ))}
                </div>
            ...
            <Button variant="gold" full disabled={!pw || !pwConfirm} onClick={handlePasswordNext}
              rightIcon={<ArrowRight size={15} />} style={{ marginTop: 'var(--space-2)' }}>
              Lanjut
            </Button>
          </div>
        </div>
      ...
      {step === 'hint' && (
        <div style={{ animation: 'fadeUp 0.3s ease both' }}>
          ...
          <input type="text" value={hint} onChange={(e) => setHint(e.target.value)}
            placeholder="Contoh: nama hewan peliharaan + tahun lahir" maxLength={100} autoFocus
            style={{ ...inputStyle(false), marginBottom: 'var(--space-5)' }} />
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            <Button variant="ghost" onClick={() => setStep('password')} leftIcon={<ArrowLeft size={14} />}>Kembali</Button>
            <Button variant="gold" style={{ flex: 1 }} onClick={() => setStep('recovery')} rightIcon={<ArrowRight size={15} />}>Lanjut</Button>
          </div>
        </div>
      )}
      ...
      {step === 'recovery' && (
        <div style={{ animation: 'fadeUp 0.3s ease both' }}>
          ...
          <textarea value={recovery} onChange={(e) => setRecovery(e.target.value)}
            placeholder="Ketikkan recovery phrase kamu… (boleh dikosongkan)" rows={3} autoFocus
            style={{ ...inputStyle(false), resize: 'none', fontFamily: 'var(--font-mono)', lineHeight: 1.7, marginBottom: 'var(--space-5)', padding: '13px 14px' }} />
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            <Button variant="ghost" onClick={() => setStep('hint')} leftIcon={<ArrowLeft size={14} />}>Kembali</Button>
            <Button variant="gold" style={{ flex: 1 }} onClick={() => setStep('done')} rightIcon={<ArrowRight size={15} />}>Lanjut</Button>
          </div>
        </div>
      )}
      ...
      {step === 'done' && (
        <div style={{ animation: 'fadeUp 0.3s ease both', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Vault siap dibuat!</h3>
          ...
          <div style={{
            textAlign: 'left', background: 'var(--bg-s1)', border: '1px solid var(--border)',
            ...
            {[
              ...
              <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 'var(--text-xs)', borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            ...
            <Button variant="gold" style={{ flex: 1 }} loading={loading} onClick={handleFinish}
...
function inputStyle(hasError: boolean): React.CSSProperties {
  return {
    ...
    background: 'var(--bg-s1)', border: `1px solid ${hasError ? 'var(--red)' : 'var(--border)'}`,
    borderRadius: 'var(--radius-md)', color: 'var(--text)', fontSize: 'var(--text-sm)',
    outline: 'none', transition: 'border-color var(--transition-fast)', boxSizing: 'border-box' as const,
  };
}
...
function ErrMsg({ msg }: { msg: string }) {
  return (
    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--red)', animation: 'fadeIn 0.2s ease', textAlign: 'center' }}>
      {msg}
    </div>
  );
}
